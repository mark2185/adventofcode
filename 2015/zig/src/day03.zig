...
fn part_one(input: []const u8) u32 {
    ...
    var visited_houses = std.AutoHashMap(Point, void).init(gpa.allocator());
...
test "examples part one" {
    ...
    for (tests) |test_case| {
        expectEqual(test_case.expected, part_one(test_case.given)) catch {
            //std.debug.print("Got {}, expected {d}\n", .{ err, test_case.expected });
        };
    }
}
...
test "examples part two" {
    ...
    for (tests) |test_case| {
        expectEqual(test_case.expected, part_two(test_case.given)) catch {
            //std.debug.print("Got {}, expected {d}\n", .{ err, test_case.expected });
        };
    }
}
