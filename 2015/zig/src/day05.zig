...
fn part_one(input: []const []const u8) usize {
    var count: usize = 0;
    for (input) |line| {
...
fn part_two(input: []const []const u8) usize {
    var count: usize = 0;
    for (input) |line| {
        if (line.len == 0) continue;
...
test "part_two examples" {
    ...
    for (test_cases) |test_case| {
        expectEqual(test_case.expected, determineMoralityAdvanced(test_case.given)) catch |err| {
            std.debug.print("Input: {s}, expected: {any}\n", .{ test_case.given, test_case.expected });
            return err;
        };
    }
}
